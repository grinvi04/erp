...
public record ApInvoiceResponse(
    ...
    public static ApInvoiceResponse from(ApInvoice inv) {
        return new ApInvoiceResponse(
            inv.getId(), inv.getInvoiceNo(),
            inv.getVendor().getId(), inv.getVendor().getName(),
            inv.getInvoiceDate(), inv.getDueDate(),
            inv.getTotalAmount(), inv.getPaidAmount(), inv.getOutstandingAmount(),
            inv.getCurrency(), inv.getStatus(),
            inv.getJournalEntryId(), inv.getApprovalRequestId(), inv.getNote());
    }
}
