...
/**
 * 재무상태표 — 연말(endDate) 기준 누적으로 자산·부채·자본 잔액을 산출하고, 당기순이익(누적 수익−비용)을 이익잉여금으로 자본에 가산해 균형(자산 ==
 * 부채+자본+당기순이익)을 확인한다.
 *
 * <p>당기순이익은 자산·부채·자본과 동일한 누적 범위(기준일 이전 전체)의 수익·비용으로 산출한다 — 복식부기 항등식(Σ차변=Σ대변)상 이 누적 순이익을 가산해야 균형식이
 * 항상 성립한다(전기 이월분 포함).
 */
@Service
  ...
  public BalanceSheetResponse getBalanceSheet(Integer year) {
    ...
    for (AccountBalanceRow r : journalLineRepository.aggregateUpTo(asOf)) {
      ...
      switch (account.getAccountType()) {
        ...
        case LIABILITY -> {
          liabilities.add(
              new BalanceSheetLine(account.getCode(), account.getName(), display(amount)));
