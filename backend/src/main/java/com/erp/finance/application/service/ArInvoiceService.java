...
public class ArInvoiceService {
  ...
  public PageResponse<ArInvoiceResponse> findByCustomer(Long customerId, Pageable pageable) {
    permissionChecker.require(Permission.FINANCE_READ);
    return PageResponse.from(
        arInvoiceRepository.findByCustomerId(customerId, pageable).map(ArInvoiceResponse::from));
  }
  ...
  public ArInvoiceResponse create(ArInvoiceCreateRequest request) {
    ...
    // 부가세예수금 통제계정 미설정이면 부가세를 분리하지 않는다(EXEMPT) — 전기 폴백과 인보이스
    // 총액·미수금(생애주기)을 일관되게 공급가액=총액으로 유지(보조원장↔GL 정합).
    TaxType effectiveTaxType =
        baseCurrencyService.currentVatAccounts().payableAccount() != null
            ? request.taxType()
            : TaxType.EXEMPT;
    ArInvoice invoice =
        ArInvoice.create(
  ...
  public ArInvoiceResponse approve(Long id) {
    ...
    auditService.record(
        ReferenceTypes.AR_INVOICE, invoice.getId(), AuditLog.AuditAction.APPROVE, null, null);
    log.atInfo()
        .addKeyValue("event", "AR_INVOICE_APPROVED")
  ...
  public ArInvoiceResponse pay(Long id, ArInvoicePayRequest request) {
    ...
    log.atInfo()
        .addKeyValue("event", "AR_INVOICE_COLLECTED")
        .addKeyValue("arInvoiceId", invoice.getId())
        .addKeyValue("invoiceNo", invoice.getInvoiceNo())
