 ...
 *
 * <ul>
 *   <li>통화 == 기준통화: amount 그대로(rate=1).
 *   <li>그 외: effectiveDate ≤ date 중 최신 환율로 amount × rate.
 *   <li>환율 부재: {@code convert}는 {@link ErrorCode#CURRENCY_RATE_NOT_FOUND}로 명시적 실패, {@code
 *       tryConvert}는 빈 Optional(거래 스냅샷의 미산정 허용 정책 — 0·1로 조용히 처리 금지).
 * </ul>
 *
...
public class CurrencyConverter implements CurrencyConversionPort {
  ...
  /**
   * 거래 시점 스냅샷용 환산 — 환율 부재 시 예외 대신 빈 Optional(거래는 정상 생성, base 미산정). amount가 null이면(예: 금액 미정
   * Opportunity) 환산 대상이 아니므로 빈 Optional.
   */
  @Override
  public Optional<Conversion> tryConvert(BigDecimal amount, String currency, LocalDate date) {
    ...
    if (baseCurrency.equals(currency)) {
      return Optional.of(
          new Conversion(amount.setScale(BASE_AMOUNT_SCALE, RoundingMode.HALF_UP), BigDecimal.ONE));
    }
            ...
            rate ->
                new Conversion(
                    amount
                        ...
                        .setScale(BASE_AMOUNT_SCALE, RoundingMode.HALF_UP),
                    rate.getRate()));
  }
