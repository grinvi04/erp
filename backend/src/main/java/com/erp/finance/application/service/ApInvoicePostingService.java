...
public class ApInvoicePostingService {
  ...
  public Long postDraft(ApInvoice invoice) {
    ...
    Account vatReceivable = baseCurrencyService.currentVatAccounts().receivableAccount();
    boolean includeVat = invoice.getVatAmount().signum() > 0 && vatReceivable != null;

    List<JournalLineRequest> lines = new ArrayList<>();
    // 차변: 비용/자산(공급가액) — 인보이스 라인별 계정
    ...
    // 차변: 부가세대급금(매입세액) — 과세·세액>0·통제계정 설정 시. 미설정이면 부가세 생략(폴백) + 로그.
    if (includeVat) {
      lines.add(
          new JournalLineRequest(
              vatReceivable.getId(),
              ...
              BigDecimal.ZERO,
              "부가세대급금: " + invoice.getInvoiceNo(),
              null));
    } else if (invoice.getVatAmount().signum() > 0) {
      log.info(
          "event=VAT_POSTING_SKIPPED type=AP invoiceNo={} reason=vat_receivable_account_not_configured",
          invoice.getInvoiceNo());
    }
    // 대변: 공급업체 외상매입금 통제계정 = 부가세 포함 시 총액, 폴백 시 공급가액(차대변 균형 보존).
    BigDecimal payableAmount = includeVat ? invoice.getTotalAmount() : invoice.getSupplyAmount();
    lines.add(
        new JournalLineRequest(
            payables.getId(),
            BigDecimal.ZERO,
