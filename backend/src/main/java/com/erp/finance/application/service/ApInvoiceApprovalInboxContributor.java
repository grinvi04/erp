...
/**
 * AP 전표 전결규정 결재를 통합 결재함에 기여한다. 전결권(finance:invoice:approve) 보유자에게
 * 자기 전결 한도 내·본인 작성 아닌 대기 전표를 보여준다 — {@link ApInvoiceService#approve}의
 * 결재 권한 기준과 동일하게 산출한다.
 */
@Component
@RequiredArgsConstructor
@Transactional(readOnly = true)
public class ApInvoiceApprovalInboxContributor implements PendingApprovalContributor {
    ...
    public List<ApprovalSummaryResponse> pendingForCurrentUser() {
        ...
        return apInvoiceRepository
                .findPendingApprovableBy(ApInvoiceStatus.PENDING_APPROVAL, userId, limit)
                .stream()
                .map(this::toSummary)
                .toList();
    }

    private ApprovalSummaryResponse toSummary(ApInvoice inv) {
        return new ApprovalSummaryResponse(
                inv.getApprovalRequestId(), ReferenceTypes.AP_INVOICE, inv.getId(),
                "AP 전표 승인: " + inv.getInvoiceNo(),
