...
public class ApInvoiceService {
  ...
  public PageResponse<ApInvoiceResponse> findAll(ApInvoiceStatus status, Pageable pageable) {
    ...
    if (status != null) {
      return PageResponse.from(
          apInvoiceRepository.findByStatus(status, pageable).map(ApInvoiceResponse::from));
    }
    return PageResponse.from(apInvoiceRepository.findAll(pageable).map(ApInvoiceResponse::from));
  }

  public PageResponse<ApInvoiceResponse> findAll(
      ApInvoiceStatus status, Long vendorId, LocalDate from, LocalDate to, Pageable pageable) {
    permissionChecker.require(Permission.FINANCE_READ);
    return PageResponse.from(
        apInvoiceRepository
            .search(status, vendorId, from, to, pageable)
            .map(ApInvoiceResponse::from));
  }

  public PageResponse<ApInvoiceResponse> findByVendor(Long vendorId, Pageable pageable) {
    permissionChecker.require(Permission.FINANCE_READ);
    return PageResponse.from(
        apInvoiceRepository.findByVendorId(vendorId, pageable).map(ApInvoiceResponse::from));
  }
  ...
  public ApInvoiceResponse create(ApInvoiceCreateRequest request) {
    ...
    currencyConverter
        .tryConvert(invoice.getTotalAmount(), invoice.getCurrency(), invoice.getInvoiceDate())
  ...
  public ApInvoiceResponse approve(Long id) {
    ...
    auditService.record(
        ReferenceTypes.AP_INVOICE, invoice.getId(), AuditLog.AuditAction.APPROVE, null, null);
    log.atInfo()
        .addKeyValue("event", "AP_INVOICE_APPROVED")
  ...
  public ApInvoiceResponse pay(Long id, ApInvoicePayRequest request) {
    ...
    if (request.cashAccountId() != null) {
      Account cashAccount =
          accountRepository
              .findById(request.cashAccountId())
              .orElseThrow(() -> new ErpException(ErrorCode.ACCOUNT_NOT_FOUND));
      cashAccount.assertPostable();
      java.time.LocalDate paymentDate =
          request.paymentDate() != null ? request.paymentDate() : invoice.getInvoiceDate();
      apInvoicePostingService.postPaymentDraft(invoice, request.amount(), cashAccount, paymentDate);
    }
    log.atInfo()
        .addKeyValue("event", "AP_INVOICE_PAID")
        .addKeyValue("apInvoiceId", invoice.getId())
        .addKeyValue("invoiceNo", invoice.getInvoiceNo())
