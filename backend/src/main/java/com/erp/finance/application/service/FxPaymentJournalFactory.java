 ...
 *
 * <p><b>환차 적용</b>(외화 AND 인보이스환율 있음 AND 결제일 환율 있음 AND 환차손익 계정 설정됨): 분개를 기준통화로 기록한다 — 통제계정@인보이스환율(원장
 * 정확 청산)·현금@결제환율·차액@환차손익 계정. AP는 결제환율&gt;인보이스환율이면 (차)환차손, 낮으면 (대)환차이익. AR은 부호 반전.
 *
 * <p><b>폴백</b>(위 조건 하나라도 불충족): 기존 원통화 2라인을 그대로 반환(환차 없음, 결제 차단 안 함) + 사유
 * 로그(event=FX_GAINLOSS_SKIPPED). 차대변은 두 반올림 금액의 차로 환차를 잡아 항상 균형.
 */
@Slf4j
@Component
...
public class FxPaymentJournalFactory {
  ...
  /**
   * 결제 분개 라인을 구성한다. {@code invoiceRate}는 인보이스 스냅샷 환율, {@code amount}는 결제(원통화) 금액. {@code control}은
   * 통제계정(AP=외상매입금/AR=외상매출금), {@code cash}는 현금·예금 계정.
   */
  public PaymentLines build(
      Side side,
      String invoiceNo,
    ...
    if (fxAccounts.isEmpty()) {
      return fallback(
          side, invoiceNo, control, cash, amount, invoiceCurrency, "FX_ACCOUNTS_NOT_CONFIGURED");
    }
  ...
  private PaymentLines fallback(
      ...
      String reason) {
    log.atInfo()
        .addKeyValue("event", "FX_GAINLOSS_SKIPPED")
