...
public class JournalEntryService {
  ...
  public PageResponse<JournalEntryResponse> findByFiscalPeriod(
      Long fiscalPeriodId, Pageable pageable) {
    permissionChecker.require(Permission.FINANCE_READ);
    return PageResponse.from(
        journalEntryRepository
            .findByFiscalPeriodId(fiscalPeriodId, pageable)
  ...
  /**
   * 권한 게이트 없는 DRAFT 분개 생성 — finance 모듈 내부의 시스템 자동 전기(AP 전표 승인 등)용. 결재 승인이 분개 생성을 인가하므로 결재자에게
   * finance:write를 요구하지 않는다(package-private).
   */
  @Transactional
  JournalEntryResponse createInternal(JournalEntryCreateRequest request) {
    FiscalPeriod period =
        fiscalPeriodRepository
            .findById(request.fiscalPeriodId())
    ...
    for (JournalLineRequest lineReq : request.lines()) {
      ...
      JournalLine line =
          JournalLine.of(
              ...
              account,
              lineReq.debitAmount(),
              lineReq.creditAmount(),
              lineReq.description(),
              lineReq.departmentId());
      entry.addLine(line);
    }
    ...
    currencyConverter
        .tryConvert(entry.getTotalDebit(), entry.getCurrency(), entry.getEntryDate())
        .ifPresent(c -> entry.applyBaseSnapshot(c.baseAmount(), c.rate()));

    JournalEntry saved = journalEntryRepository.save(entry);
    log.atInfo()
        .addKeyValue("event", "JOURNAL_ENTRY_CREATED")
  ...
  /**
   * 결재 상신: DRAFT → PENDING_APPROVAL + ApprovalRequest(GL_ENTRY) 생성·링크. 차대변 균형·회계기간 검증은 도메인({@link
   * JournalEntry#submitForApproval})에서 수행. 결재함 라우팅은 {@link GlEntryApprovalInboxContributor}가
   * 전결권·한도로 산출한다(역할 sentinel).
   */
  @Transactional
  public JournalEntryResponse submitForApproval(Long id) {
    ...
    log.atInfo()
        .addKeyValue("event", "JOURNAL_ENTRY_SUBMITTED")
  ...
  /**
   * 전기 결재: 전결권(finance:gl:approve) 보유자만, 작성자≠결재자·차변≤전결한도 충족 시 ApprovalRequest 승인 후 PENDING_APPROVAL
   * → POSTED 전기. 작성권(finance:write)과 분리.
   */
  @Transactional
  public JournalEntryResponse approve(Long id) {
    ...
    entry.post(userId);
    auditService.record(
        ReferenceTypes.GL_ENTRY, entry.getId(), AuditLog.AuditAction.APPROVE, null, null);
    log.atInfo()
        .addKeyValue("event", "JOURNAL_ENTRY_POSTED")
        .addKeyValue("journalEntryId", entry.getId())
        .addKeyValue("entryNo", entry.getEntryNo())
  ...
  /**
   * 전기 결재 반려: 전결권(finance:gl:approve) 보유자만, 작성자≠결재자 충족 시 반려 사유와 함께 ApprovalRequest를 REJECTED로 종료하고
   * 전표를 PENDING_APPROVAL → DRAFT로 되돌린다(수정·재상신 가능).
   */
  @Transactional
  public JournalEntryResponse reject(Long id, String comment) {
    ...
    auditService.record(
        ReferenceTypes.GL_ENTRY, entry.getId(), AuditLog.AuditAction.REJECT, null, null);
    log.atInfo()
        .addKeyValue("event", "JOURNAL_ENTRY_REJECTED")
        .addKeyValue("journalEntryId", entry.getId())
        .addKeyValue("entryNo", entry.getEntryNo())
        .log("전표 결재 반려");
    return JournalEntryResponse.from(entry);
  ...
  /**
   * 전기 결재 철회: 상신자 본인(작성권 finance:write)만, ApprovalRequest를 CANCELLED로 종료하고 전표를 PENDING_APPROVAL →
   * DRAFT로 되돌린다(수정·재상신 가능). 타인은 철회할 수 없다.
   */
  @Transactional
  public JournalEntryResponse withdraw(Long id) {
    ...
    auditService.record(
        ReferenceTypes.GL_ENTRY, entry.getId(), AuditLog.AuditAction.WITHDRAW, null, null);
    log.atInfo()
        .addKeyValue("event", "JOURNAL_ENTRY_WITHDRAWN")
        .addKeyValue("journalEntryId", entry.getId())
        .addKeyValue("entryNo", entry.getEntryNo())
        .log("전표 결재 철회");
    return JournalEntryResponse.from(entry);
  ...
  private String generateEntryNo(LocalDate date) {
    ...
    for (int i = 0; i < ENTRY_NO_MAX_RETRIES; i++) {
      int suffix =
          ThreadLocalRandom.current().nextInt(ENTRY_NO_SUFFIX_MIN, ENTRY_NO_SUFFIX_MAX + 1);
