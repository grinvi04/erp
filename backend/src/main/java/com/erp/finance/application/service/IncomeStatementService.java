...
/**
 * 손익계산서 — 회계연도 기간의 REVENUE(대변−차변)·EXPENSE(차변−대변) 발생액과 당기순이익(수익−비용)을 산출한다. 부호는 계정 normalBalance 기준으로
 * 처리해 수익·비용 모두 양수로 표시된다.
 */
@Service
  ...
  public IncomeStatementResponse getIncomeStatement(Integer year) {
    ...
    for (AccountBalanceRow r :
      ...
      if (account.getAccountType() == AccountType.REVENUE) {
        revenues.add(
            new IncomeStatementLine(account.getCode(), account.getName(), display(amount)));
    ...
    return new IncomeStatementResponse(
        ...
        display(totalRevenue.subtract(totalExpense)),
        excluded);
  }
}
