 ...
 *
 * 차변 외상매출금은 전표가 아니라 고객 마스터의 통제계정에서 온다(AR 보조원장 ↔ GL 일치). 전기(POST)는 회계담당이 별도 수행한다(여기선 DRAFT만 생성).
 *
...
public class ArInvoicePostingService {
   ...
   *
   * <ul>
   *   <li>차변: 고객 외상매출금 통제계정 = 총액
   *   <li>대변: 각 라인 계정(매출·부가세예수금 등) = 라인 금액
   * </ul>
   */
  @Transactional
  ...
  /**
   * 수금 시 DRAFT 분개(AP 지급의 반전):
   *
   * <pre>(차) 현금·예금 [수금계정]  (대) 외상매출금 [고객 통제계정] = 수금액</pre>
   *
   * 외상매출금 통제계정·수금계정이 없으면 전기하지 않고 {@code null}을 반환한다.
   *
   * <p>외화 수금으로 수금환율이 인보이스 스냅샷 환율과 다르면 실현 환차손익 라인을 추가해 기준통화로 기록한다 (AP 지급의 부호 반전 — 더 수금 시 환차이익, 덜 수금
   * 시 환차손). 조건 미충족 시 원통화 2라인으로 폴백 ({@link FxPaymentJournalFactory}).
   */
  @Transactional
  public Long postPaymentDraft(
      ArInvoice invoice, BigDecimal amount, Account cashAccount, LocalDate paymentDate) {
    Account receivables = invoice.getCustomer().getReceivablesAccount();
    ...
    FxPaymentJournalFactory.PaymentLines payment =
        fxPaymentJournalFactory.build(
            FxPaymentJournalFactory.Side.AR,
