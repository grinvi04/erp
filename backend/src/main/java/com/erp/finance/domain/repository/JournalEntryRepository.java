...
public interface JournalEntryRepository extends JpaRepository<JournalEntry, Long> {
  ...
  @Query(
      value =
          "SELECT j FROM JournalEntry j LEFT JOIN FETCH j.fiscalPeriod p WHERE p.id = :fiscalPeriodId",
      countQuery = "SELECT COUNT(j) FROM JournalEntry j WHERE j.fiscalPeriod.id = :fiscalPeriodId")
  Page<JournalEntry> findByFiscalPeriodId(
      @Param("fiscalPeriodId") Long fiscalPeriodId, Pageable pageable);

  @Query(
      value =
          "SELECT j FROM JournalEntry j LEFT JOIN FETCH j.fiscalPeriod WHERE j.status = :status",
  ...
  @Query(
      value =
          "SELECT j FROM JournalEntry j LEFT JOIN FETCH j.fiscalPeriod p "
  ...
  Page<JournalEntry> findByFiscalPeriodIdAndStatus(
  ...
  /**
   * 환율 미산정(exchangeRate null) POSTED 분개 수(기간) — 재무제표 집계에서 제외된 건수를 응답에 표기하기 위함(조용한 누락 금지).
   * 시산표·손익계산서용.
   */
  @Query(
      "SELECT COUNT(j) FROM JournalEntry j "
  ...
  long countPostedWithoutRateBetween(
  ...
  /**
   * 전결규정상 현재 사용자가 전기 결재할 수 있는 대기 전표 — 통합 결재함 라우팅용. 상태=대기, 작성자≠본인(직무분리), 차변 합계≤본인 전결 한도. 테넌트 필터는 자동
   * 적용.
   */
  @Query(
      "SELECT j FROM JournalEntry j LEFT JOIN FETCH j.fiscalPeriod "
          + "WHERE j.status = :status AND j.createdBy <> :userId AND j.totalDebit <= :limit")
  java.util.List<JournalEntry> findPendingApprovableBy(
      @Param("status") JournalEntryStatus status,
