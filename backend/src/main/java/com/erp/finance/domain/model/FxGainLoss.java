 ...
 *
 * <ul>
 *   <li>{@code controlAmount} = paidAmount × invoiceRate — 통제계정(외상매입/매출금) 청산액(인보이스환율).
 *   <li>{@code cashAmount} = paidAmount × paymentRate — 현금 환산액(결제환율).
 * </ul>
 *
 * <p>차액은 두 <b>반올림된</b> 금액의 차(=환차)로 잡아 분개 차대변이 정확히 균형하도록 한다 (양쪽을 각각 ROUND(2) 후 빼므로 잔돈이 남지 않는다). 부호
 * 해석(손익·차대변)은 AP/AR 호출부에서 {@link #cashExceedsControl()}로 결정한다. 순수 계산 — 부수효과·의존 없음(단위 테스트 가능).
 */
public record FxGainLoss(BigDecimal controlAmount, BigDecimal cashAmount) {
  ...
  public static FxGainLoss of(
      BigDecimal paidAmount, BigDecimal invoiceRate, BigDecimal paymentRate) {
  ...
  /** 현금 환산액이 통제계정 청산액보다 큰지 — 결제환율 &gt; 인보이스환율. AP면 더 지급(환차손), AR이면 더 수금(환차이익). 반대면 각각 환차이익·환차손. */
