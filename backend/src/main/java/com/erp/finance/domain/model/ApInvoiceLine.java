...
/**
 * AP 전표 라인 — 차변 계정(비용/자산·부가세대급금 등)과 금액. 승인 시 GL 분개의 차변이 된다
 * (대변은 공급업체 외상매입금 통제계정). 실무: 비용/세액을 라인별 계정으로 코딩한다.
 */
@Entity
...
public class ApInvoiceLine extends BaseEntity {
    ...
    @SequenceGenerator(name = "ap_invoice_line_seq",
        sequenceName = "finance.ap_invoice_line_id_seq", allocationSize = 50)
    private Long id;
    ...
    static ApInvoiceLine of(ApInvoice apInvoice, int lineNo, Account account,
