...
/**
 * frankfurter.app(ECB 기반, 키리스) 환율 조회 어댑터 — API 키·시크릿 없음. 응답 예: GET /{date}?from=USD&to=KRW →
 * {"amount":1.0,"base":"USD","date":"2024-01-02","rates":{"KRW":1300.5}} 실패 시(네트워크·미지원 통화·파싱 오류) 빈
 * Optional 반환 → 호출 측이 수동 환율로 graceful fallback.
 */
@Slf4j
@Component
public class FrankfurterExchangeRateProvider implements ExchangeRateProvider {
  ...
  record FrankfurterResponse(
      BigDecimal amount, String base, String date, Map<String, Object> rates) {}
}
