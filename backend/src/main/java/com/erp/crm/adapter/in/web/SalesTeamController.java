...
public class SalesTeamController {
  ...
  public ResponseEntity<ApiResponse<SalesTeamResponse>> update(
      @PathVariable Long id, @Valid @RequestBody SalesTeamUpdateRequest request) {
  ...
  public ResponseEntity<ApiResponse<SalesTeamResponse>> addMember(
    ...
    return ResponseEntity.ok(ApiResponse.ok(salesTeamService.addMember(id, request.userId())));
  }

  @DeleteMapping("/{id}/members/{userId}")
  public ResponseEntity<ApiResponse<SalesTeamResponse>> removeMember(
      @PathVariable Long id, @PathVariable String userId) {
    return ResponseEntity.ok(ApiResponse.ok(salesTeamService.removeMember(id, userId)));
  }
}
