...
/**
 * 영업팀(CRM 영업조직) — DataScope의 DEPARTMENT(팀) 스코프 기준. 팀원(사용자 sub)을 묶어,
 * DEPARTMENT 스코프 사용자가 같은 팀 팀원들의 영업데이터(owner 기준)를 볼 수 있게 한다.
 * 부서(HR)와 독립한 CRM 자체 평면 팀 모델 — 모듈 경계 유지.
 */
@Entity
...
public class SalesTeam extends BaseEntity {
    ...
    @SequenceGenerator(name = "sales_team_seq", sequenceName = "crm.sales_team_id_seq", allocationSize = 50)
    private Long id;
