...
/**
 * 영업팀(SalesTeam) 관리 — 팀과 팀원(사용자 sub)을 관리한다. CRM DataScope의 DEPARTMENT 스코프
 * 기준이 되는 조직 데이터이므로, 읽기는 iam:read·쓰기는 iam:write 권한을 요구하고(IamService 패턴),
 * 변경은 감사 로그에 남긴다. 테넌트 격리는 BaseEntity의 @TenantId 자동필터에 위임한다.
 */
@Service
...
public class SalesTeamService {
    ...
    public List<SalesTeamResponse> listTeams() {
        ...
        return salesTeamRepository.findAllByOrderByCodeAsc().stream()
                .map(SalesTeamResponse::from).toList();
    }
    ...
    public SalesTeamResponse createTeam(String code, String name) {
        ...
        auditService.record("SALES_TEAM", saved.getId(), AuditLog.AuditAction.CREATE, null,
    ...
    public SalesTeamResponse updateTeam(Long id, String name, Long version) {
        ...
        team.rename(name);
        auditService.record("SALES_TEAM", team.getId(), AuditLog.AuditAction.UPDATE, null,
    ...
    public void deleteTeam(Long id) {
        ...
        auditService.record("SALES_TEAM", id, AuditLog.AuditAction.DELETE, null,
                json(Map.of("code", team.getCode())));
    }
    ...
    public SalesTeamResponse addMember(Long teamId, String userId) {
        ...
        auditService.record("SALES_TEAM", teamId, AuditLog.AuditAction.UPDATE, null,
                json(Map.of("addMember", userId)));
        return SalesTeamResponse.from(team);
    }
