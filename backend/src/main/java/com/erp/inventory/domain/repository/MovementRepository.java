...
public interface MovementRepository extends JpaRepository<Movement, Long> {
  ...
  @Query(
      "SELECT m.movementType AS movementType, COUNT(m.id) AS count "
   ...
   */
  @Query(
      "SELECT m FROM Movement m WHERE m.status = :status "
          + "AND m.movementType = com.erp.inventory.domain.model.MovementType.ADJUSTMENT "
          + "AND m.createdBy <> :userId")
  List<Movement> findPendingApprovableBy(
      @Param("status") MovementStatus status, @Param("userId") String userId);
}
