...
public class Stock extends BaseEntity {
  ...
  @SequenceGenerator(
      name = "stock_seq",
      sequenceName = "inventory.stock_id_seq",
      allocationSize = 100)
  private Long id;
  ...
  public static Stock create(
      Item item, Location location, String lotNo, String serialNo, BigDecimal unitCost) {
    Stock s = new Stock();
    s.item = item;
    s.location = location;
    s.lotNo = lotNo;
  ...
  public void increase(BigDecimal qty, BigDecimal incomingCost) {
    BigDecimal totalValue = this.qtyOnHand.multiply(this.unitCost).add(qty.multiply(incomingCost));
