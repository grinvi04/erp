...
public class MovementService {
  ...
  private static final String ROLE_BASED_APPROVER =
      "@role:" + Permission.INVENTORY_MOVEMENT_APPROVE;
  ...
  public PageResponse<MovementResponse> findAll(
    ...
    Map<Long, List<MovementLineResponse>> linesMap =
        movementLineRepository.findByMovement_IdInOrderByLineNoAsc(ids).stream()
            .collect(
                Collectors.groupingBy(
                    l -> l.getMovement().getId(),
                    Collectors.mapping(MovementLineResponse::from, Collectors.toList())));
    return PageResponse.from(
        page.map(m -> MovementResponse.from(m, linesMap.getOrDefault(m.getId(), List.of()))));
  }
  ...
  public MovementResponse create(MovementCreateRequest req) {
    ...
    List<MovementLineResponse> lineResponses =
        lines.stream().map(MovementLineResponse::from).toList();
  ...
  public MovementResponse confirm(Long id) {
    ...
    log.atInfo()
        .addKeyValue("event", "STOCK_MOVEMENT_CONFIRMED")
  ...
  /**
   * 재고 조정 결재 상신: ADJUSTMENT DRAFT → PENDING_APPROVAL + ApprovalRequest(STOCK_MOVEMENT) 생성·링크. 조정
   * 이동만 결재 대상이며(도메인 가드), 결재함 라우팅은 {@link StockMovementApprovalInboxContributor}가 권한(역할 sentinel)으로
   * 산출한다.
   */
  @Transactional
  public MovementResponse submitForApproval(Long id) {
    ...
    log.atInfo()
        .addKeyValue("event", "STOCK_MOVEMENT_SUBMITTED")
        .addKeyValue("movementId", movement.getId())
        .addKeyValue("movementNo", movement.getMovementNo())
        .log("재고 조정 이동 결재 상신");
    return toResponse(id, movement);
  ...
  /**
   * 재고 조정 확정 결재: 결재권(inventory:movement:approve) 보유자만, 작성자≠결재자 충족 시 ApprovalRequest 승인 후
   * PENDING_APPROVAL → CONFIRMED + 재고 증감 반영. 작성권(inventory:write)과 분리.
   */
  @Transactional
  public MovementResponse approve(Long id) {
    ...
    auditService.record(
        ReferenceTypes.STOCK_MOVEMENT, movement.getId(), AuditLog.AuditAction.APPROVE, null, null);
    log.atInfo()
        .addKeyValue("event", "STOCK_MOVEMENT_APPROVED")
        .addKeyValue("movementId", movement.getId())
        .addKeyValue("movementNo", movement.getMovementNo())
        .log("재고 조정 이동 결재 승인");
    return MovementResponse.from(movement, lines.stream().map(MovementLineResponse::from).toList());
  ...
  private List<MovementLine> applyStockEffects(Long movementId) {
    ...
    for (MovementLine line : lines) {
      if (line.getFromLocation() != null) {
        Stock stock =
            stockRepository
                .findByItemAndLocationAndLotNoAndSerialNo(
  ...
  private List<MovementLine> buildLines(Movement movement, MovementCreateRequest req) {
    ...
    for (int i = 0; i < lineReqs.size(); i++) {
      ...
      Location fromLocation =
          lineReq.fromLocationId() != null
              ? locationService.getOrThrow(lineReq.fromLocationId())
              : null;
      Location toLocation =
          lineReq.toLocationId() != null
              ? locationService.getOrThrow(lineReq.toLocationId())
              : null;
  ...
  private String generateMovementNo() {
    ...
    for (int i = 0; i < MOVEMENT_NO_MAX_RETRIES; i++) {
      int suffix =
          ThreadLocalRandom.current().nextInt(MOVEMENT_NO_SUFFIX_MIN, MOVEMENT_NO_SUFFIX_MAX + 1);
  ...
  /**
   * 재고 조정 결재 반려: 결재권(inventory:movement:approve) 보유자만, 작성자≠결재자 충족 시 반려 사유와 함께 ApprovalRequest를
   * REJECTED로 종료하고 이동을 PENDING_APPROVAL → DRAFT로 되돌린다(수정·재상신 가능).
   */
  @Transactional
  public MovementResponse reject(Long id, String comment) {
    ...
    auditService.record(
        ReferenceTypes.STOCK_MOVEMENT, movement.getId(), AuditLog.AuditAction.REJECT, null, null);
    log.atInfo()
        .addKeyValue("event", "STOCK_MOVEMENT_REJECTED")
        .addKeyValue("movementId", movement.getId())
        .addKeyValue("movementNo", movement.getMovementNo())
        .log("재고 조정 이동 결재 반려");
    return toResponse(id, movement);
  ...
  /**
   * 재고 조정 결재 철회: 상신자 본인(작성권 inventory:write)만, ApprovalRequest를 CANCELLED로 종료하고 이동을
   * PENDING_APPROVAL → DRAFT로 되돌린다(수정·재상신 가능). 타인은 철회할 수 없다.
   */
  @Transactional
  public MovementResponse withdraw(Long id) {
    ...
    auditService.record(
        ReferenceTypes.STOCK_MOVEMENT, movement.getId(), AuditLog.AuditAction.WITHDRAW, null, null);
    log.atInfo()
        .addKeyValue("event", "STOCK_MOVEMENT_WITHDRAWN")
        .addKeyValue("movementId", movement.getId())
        .addKeyValue("movementNo", movement.getMovementNo())
        .log("재고 조정 이동 결재 철회");
    return toResponse(id, movement);
  ...
  private MovementResponse toResponse(Long id, Movement movement) {
    List<MovementLineResponse> lines =
        movementLineRepository.findByMovement_IdOrderByLineNoAsc(id).stream()
            .map(MovementLineResponse::from)
            .toList();
