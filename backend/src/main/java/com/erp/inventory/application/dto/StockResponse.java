...
public record StockResponse(
    ...
    public static StockResponse from(Stock s) {
        return new StockResponse(
                s.getId(),
                s.getItem().getId(), s.getItem().getSku(), s.getItem().getName(),
                s.getLocation().getId(), s.getLocation().getCode(), s.getLocation().getName(),
                s.getLocation().getWarehouse().getId(), s.getLocation().getWarehouse().getName(),
                s.getLotNo(), s.getSerialNo(),
                s.getQtyOnHand(), s.getQtyReserved(), s.getUnitCost());
    }
}
