...
/**
 * 부트스트랩 — 인가를 DB로 전환하면 초기엔 누구도 iam:write 권한이 없어 관리 API를 쓸 수 없다
 * (chicken-and-egg). 설정된 관리자(sub)에게 모든 권한을 가진 SUPER_ADMIN 역할을 기동 시 보장한다.
 * 멱등 — 재기동 시 권한 카탈로그가 늘어나면 역할에 반영한다. {@code erp.iam.bootstrap.admin-sub}
 * 미설정이면 아무 것도 하지 않는다. 테스트 프로파일에선 비활성(테스트는 자체 시드).
 */
@Component
@Profile("!test")
...
public class IamBootstrap implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        Role role = roleRepository.findByTenantIdAndCode(tenantId, SUPER_ADMIN)
