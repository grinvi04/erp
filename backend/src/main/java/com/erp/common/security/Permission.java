...
/**
 * 기능 권한 코드 — {모듈}:{리소스}:{액션}. (auth-standards: 코드는 권한을 검사하고 역할은 운영이 관리 화면에서 조합한다 — 코드에 역할명 하드코딩 금지)
 *
 * <p>권한 부여는 JWT의 {@code permissions} 클레임으로 전달되며(Keycloak이 역할→권한 매핑을 발급), {@link
 * JwtAuthoritiesConverter}가 authority로 변환한다. 도메인 결재선 (승인 권한)은 RBAC가 아니라 도메인 업무 규칙으로
 * 구현한다(auth-standards).
 */
public final class Permission {
  ...
  // 재무 기준정보(기준통화·환율) 변경권. 조회(finance:read)와 분리 — 기준통화·환율은 환산 합계의 기준이므로 관리자만 변경. 기존 write 보유 역할엔
  // V0007로 백필.
  ...
  // 재고 조정(ADJUSTMENT) 이동 확정 결재권. 이동 작성(inventory:write)과 분리 — 작성자≠확정결재자 직무분리. 기존 write 보유 역할엔
  // V0006으로 백필.
  public static final String INVENTORY_MOVEMENT_APPROVE = "inventory:movement:approve";
   ...
   * IamPermissionCatalogTest}가 누락을 막는다.
   */
