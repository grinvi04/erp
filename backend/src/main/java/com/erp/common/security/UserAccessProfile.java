...
/**
 * 사용자별 접근 프로파일 — 데이터 스코프(ALL/DEPARTMENT/SELF)·소속 부서·전결 한도(원).
 * 기존 JWT 클레임(data_scope·department_id·approval_limit)을 대체하는 DB 정본.
...
public class UserAccessProfile {
    ...
    @SequenceGenerator(name = "user_access_profile_seq",
        sequenceName = "common.user_access_profile_id_seq", allocationSize = 50)
    private Long id;
    ...
    public static UserAccessProfile of(Long tenantId, String userId, DataScope dataScope,
                                       Long departmentId, BigDecimal approvalLimit) {
