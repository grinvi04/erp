...
/**
 * 역할 — 권한 코드의 묶음(운영이 관리 화면에서 조합). 코드는 권한을 검사하고, 역할은 그 권한 집합을 사용자에게 부여하는 단위다(auth-standards). {@link
 * com.erp.common.audit.AuditLog}와 동일하게 {@code BaseEntity}/@TenantId를 쓰지 않고 tenant_id를 명시 필터링한다 — 권한
 * 해석이 JWT 인증 단계(TenantContext 세팅 전)에서 일어나기 때문.
 */
@Entity
...
public class Role {
  ...
  public void grant(String permissionCode) {
    boolean already =
        permissions.stream().anyMatch(p -> p.getPermissionCode().equals(permissionCode));
    if (!already) {
  ...
  /**
   * 권한 집합을 주어진 코드들로 교체한다. 사라진 코드만 제거하고 새 코드만 추가해 유지되는 행은 건드리지 않는다 — 삭제 후 재삽입이 (role_id,
   * permission_code) UNIQUE를 일시적으로 위반하는 것을 피한다.
   */
