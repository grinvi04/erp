...
/**
 * 제네릭 결재 요청 — HR 휴가, Finance AP 전표, 구매 발주 등 모든 결재 흐름에 사용.
 * 결재 대상 도메인은 entity_type + entity_id로 참조 (직접 FK 없음 — 모듈 경계 유지).
 */
@Entity
...
public class ApprovalRequest extends BaseEntity {
    ...
    @SequenceGenerator(name = "approval_request_seq", sequenceName = "common.approval_request_id_seq", allocationSize = 50)
    private Long id;
    ...
    public static ApprovalRequest create(String entityType, Long entityId, String title,
                                         String requesterId, List<ApprovalStep> steps) {
