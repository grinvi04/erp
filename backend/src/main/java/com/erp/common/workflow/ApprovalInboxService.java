...
/** 통합 결재함 — 여러 모듈의 결재 요청을 한 곳에서 조회한다. 읽기 전용: 실제 승인/반려는 각 도메인(모듈) 서비스가 소유한다(모듈 경계 유지). */
@Service
...
public class ApprovalInboxService {
  ...
  /**
   * 현재 사용자가 처리해야 할 대기 결재 목록. 사람 단위 결재선(현재 단계 결재자=나)에 더해, 역할·금액 한도 기반 결재(모듈 기여분)를 합친다.
   * (entityType,entityId)로 중복 제거.
   */
