 ...
 *
 * <p>역할·금액 한도 기반 결재(예: AP 전표 전결규정)는 사람 단위 결재선(person-assigned
 * {@code ApprovalStep})으로 표현되지 않으므로, {@code findPendingForApprover}(결재자=나)
 * 쿼리로는 라우팅되지 않는다. 각 모듈이 자기 도메인 규칙으로 대기 목록을 산출해 결재함에
 * 제공한다 — 결재함(common)은 모듈 규칙을 알 필요가 없다(모듈 경계 유지).
 */
