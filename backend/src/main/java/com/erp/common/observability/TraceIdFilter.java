 ...
 *
 * <p>인바운드 W3C Trace Context {@code traceparent} 헤더에서 trace-id를 추출해 재사용하고,
 * 없거나 형식이 유효하지 않으면 새로 생성한다. trace-id는 MDC에 넣어 모든 로그에 자동 포함되게
 * 하고, 응답 {@code X-Trace-Id} 헤더로 반환해 클라이언트가 역추적할 수 있게 한다.
 *
...
public class TraceIdFilter extends OncePerRequestFilter {
    ...
    /**
     * W3C {@code traceparent} 헤더에서 trace-id(32 hex)를 추출한다.
     * 형식: {@code version-traceid-spanid-flags} (예 {@code 00-4bf92f35...-00f0...-01}).
     * 유효한 trace-id(32 hex, all-zero 아님)가 있으면 그대로 쓰고, 아니면 새로 생성한다.
     */
