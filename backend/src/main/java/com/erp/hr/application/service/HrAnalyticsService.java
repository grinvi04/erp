...
/**
 * HR 분석 집계 — CrmAnalytics/FinanceAnalytics 청사진 복제. 기능 권한 + 데이터 스코프를 적용한다.
 * 분포 지표는 enum 전체를 0채움, 부서/직위는 빈 그룹을 count=0으로 보존, 월별은 1~12월 0채움.
 */
@Service
...
public class HrAnalyticsService {
    ...
    public List<EmployeeStatusCountResponse> getStatusDistribution() {
        ...
        Map<EmployeeStatus, Long> countMap =
                employeeRepository.statusDistribution(s.unscoped(), s.selfUserId(), s.deptIds()).stream()
                        .collect(Collectors.toMap(EmployeeStatusCountRow::getStatus, EmployeeStatusCountRow::getCount));
    ...
    public List<DepartmentHeadcountResponse> getHeadcountByDepartment() {
        ...
        return departmentRepository.headcountByDepartment(s.unscoped(), s.deptIds()).stream()
                .map(r -> new DepartmentHeadcountResponse(r.getDepartmentId(), r.getDepartmentName(), r.getCount()))
                .collect(Collectors.toList());
    }
    ...
    public List<PositionHeadcountResponse> getHeadcountByPosition() {
        permissionChecker.require(Permission.HR_EMPLOYEE_READ);
        EmployeeAnalyticsScope s = dataScopeResolver.employeeAnalyticsScope();
        return positionRepository.headcountByPosition(s.unscoped(), s.selfUserId(), s.deptIds()).stream()
                .map(r -> new PositionHeadcountResponse(r.getPositionId(), r.getPositionName(), r.getCount()))
                .collect(Collectors.toList());
    }
    ...
    public List<EmploymentTypeCountResponse> getEmploymentTypeDistribution() {
        ...
        Map<EmploymentType, Long> countMap =
                employeeRepository.employmentTypeDistribution(s.unscoped(), s.selfUserId(), s.deptIds()).stream()
                        .collect(Collectors.toMap(EmploymentTypeCountRow::getEmploymentType, EmploymentTypeCountRow::getCount));
