...
public class LeaveRequestService {
  ...
  public Page<LeaveRequestResponse> findAll(Pageable pageable) {
    ...
    return leaveRequestRepository
        .findAll(dataScopeResolver.leaveRequestScope(), pageable)
  ...
  public LeaveRequestResponse create(LeaveRequestCreateRequest request) {
    ...
    LeavePolicy policy =
        leavePolicyRepository
            .findById(request.leavePolicyId())
    ...
    LeaveBalance balance =
        leaveBalanceRepository
            .findByEmployeeIdAndLeavePolicyIdAndYear(
                request.employeeId(), request.leavePolicyId(), year)
    ...
    // 결재자 = 신청자의 매니저(의 Keycloak sub). approve()의 검증도 sub 기준이므로 매니저의
    // employeeNo가 아니라 연결된 user_id를 결재자로 지정해야 실제 로그인 사용자가 결재할 수 있다.
    // 매니저 또는 매니저의 로그인 계정이 없어 결재자를 해소하지 못하면 제출을 차단한다 —
    // SYSTEM 같은 리터럴로 자동지정하면 누구의 sub와도 일치하지 못해 조용한 데드엔드가 된다.
    Employee manager = employee.getManager();
    if (manager == null || manager.getUserId() == null) {
      throw new ErpException(ErrorCode.APPROVER_NOT_RESOLVED);
    }
    ApprovalStep step = ApprovalStep.of(1, "직속 상관 승인", manager.getUserId());
  ...
  public LeaveRequestResponse approve(Long leaveRequestId, ApprovalActionRequest request) {
    ...
    if (approvalRequest.getStatus() == ApprovalStatus.APPROVED) {
      ...
      LeaveBalance balance =
          leaveBalanceRepository
              .findByEmployeeIdAndLeavePolicyIdAndYear(
                  leaveRequest.getEmployee().getId(), leaveRequest.getLeavePolicy().getId(), year)
              .orElseThrow(() -> new ErpException(ErrorCode.LEAVE_BALANCE_NOT_FOUND));
      balance.deduct(leaveRequest.getRequestedDays());
    }

    auditService.record(
        "LEAVE_REQUEST", leaveRequest.getId(), AuditLog.AuditAction.APPROVE, null, null);
    return LeaveRequestResponse.from(leaveRequest);
  }
  ...
  public LeaveRequestResponse reject(Long leaveRequestId, ApprovalActionRequest request) {
    ...
    auditService.record(
        "LEAVE_REQUEST", leaveRequest.getId(), AuditLog.AuditAction.REJECT, null, null);
    return LeaveRequestResponse.from(leaveRequest);
  }

  /**
   * 결재 권한 검증. (1) 인증된 사용자여야 하고 — null 행위자를 SYSTEM으로 승격해 우회를 허용하지 않는다, (2) 현재 단계의 결재자(매니저 sub)와 일치해야
   * 하며, (3) 본인이 상신한 건은 결재할 수 없다(직무 분리). AP 전표 결재와 동일한 기준.
   */
