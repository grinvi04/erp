...
/**
 * HR 직원 조회의 데이터 스코프 필터 — 모든 직원 조회 진입점이 공통으로 사용한다 (auth-standards: 화면별 수작업 필터 금지, 리포지토리 공통 필터).
 * 부서(common 미참조)는 HR 개념이라 이 resolver가 HR 모듈에 위치한다.
 */
@Component
  ...
  public Specification<LeaveRequest> leaveRequestScope() {
    ...
    return switch (scope) {
      ...
      case DEPARTMENT -> {
        ...
        Set<Long> deptIds = selfAndDescendantDeptIds(deptId);
        yield (root, query, cb) -> root.get("employee").get("department").get("id").in(deptIds);
      }
    };
  }

  /**
   * 집계(GROUP BY) 쿼리용 파라미터 스코프 — {@link Specification}을 쓸 수 없는 @Query 집계에 결합한다. 쿼리는 {@code
   * (:unscoped = true OR e.userId = :selfUserId OR e.department.id IN :deptIds)}로 적용. 빈 컬렉션 IN·널
   * 비교를 피하려고 deptIds는 항상 비지 않게(매칭 없는 센티넬 {@code -1L}), selfUserId는 항상 널이 아니게(매칭 없는 빈 문자열) 정규화한다. 휴가
   * 집계는 {@code lr.employee.*} 경로로 동일 적용.
   */
