...
public interface EmployeeRepository extends JpaRepository<Employee, Long>, JpaSpecificationExecutor<Employee> {
    ...
    @Query("SELECT e.status AS status, COUNT(e.id) AS count FROM Employee e "
            + "WHERE (:unscoped = true OR e.userId = :selfUserId OR e.department.id IN :deptIds) "
            + "GROUP BY e.status")
    List<EmployeeStatusCountRow> statusDistribution(@Param("unscoped") boolean unscoped,
                                                    @Param("selfUserId") String selfUserId,
                                                    @Param("deptIds") Collection<Long> deptIds);

    @Query("SELECT e.employmentType AS employmentType, COUNT(e.id) AS count FROM Employee e "
            + "WHERE (:unscoped = true OR e.userId = :selfUserId OR e.department.id IN :deptIds) "
            + "GROUP BY e.employmentType")
    List<EmploymentTypeCountRow> employmentTypeDistribution(@Param("unscoped") boolean unscoped,
                                                            @Param("selfUserId") String selfUserId,
                                                            @Param("deptIds") Collection<Long> deptIds);

    @Query("SELECT EXTRACT(MONTH FROM e.hireDate) AS month, COUNT(e.id) AS count FROM Employee e "
    ...
    List<MonthlyCountRow> monthlyHires(@Param("year") int year,
                                       @Param("unscoped") boolean unscoped,
                                       @Param("selfUserId") String selfUserId,
                                       @Param("deptIds") Collection<Long> deptIds);

    @Query("SELECT EXTRACT(MONTH FROM e.terminationDate) AS month, COUNT(e.id) AS count FROM Employee e "
    ...
    List<MonthlyCountRow> monthlyTerminations(@Param("year") int year,
                                              @Param("unscoped") boolean unscoped,
                                              @Param("selfUserId") String selfUserId,
                                              @Param("deptIds") Collection<Long> deptIds);
}
