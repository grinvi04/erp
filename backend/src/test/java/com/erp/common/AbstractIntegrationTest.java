...
public abstract class AbstractIntegrationTest {
  ...
  /**
   * 테스트용 인증 컨텍스트를 설정한다 — subject·sub·tenant_id 클레임을 가진 JWT와 전달된 권한 코드를 SecurityContext에 넣는다. 데이터
   * 스코프·결재한도 등 DB 프로파일이 필요한 테스트는 이 헬퍼로 신원을 세우고 프로파일 저장은 각 테스트에서 수행한다.
   */
  protected void authenticate(String sub, String... permissions) {
    Jwt jwt =
        Jwt.withTokenValue("t")
            ...
            .subject(sub)
            .claim("sub", sub)
            .claim("tenant_id", TEST_TENANT_ID)
            .build();
