...
class ApprovalInboxQueryIntegrationTest extends AbstractIntegrationTest {
  ...
  private ApprovalRequest pending(
      String entityType, Long entityId, String requester, String... stepApprovers) {
    ...
    return approvalRequestRepository.save(
        ApprovalRequest.create(
            entityType, entityId, entityType + " 결재", requester, new ArrayList<>(List.of(steps))));
  }
  ...
  void findPendingForApprover_returnsOnlyRequestsWhereCurrentStepApproverMatches() {
    ...
    assertThat(result).extracting(ApprovalRequest::getEntityId).containsExactlyInAnyOrder(1L, 3L);
  ...
  void findByRequesterIdOrderByRequestedAtDesc_returnsOnlyMyRequests() {
    ...
    List<ApprovalRequest> mine =
        approvalRequestRepository.findByRequesterIdOrderByRequestedAtDesc("me");

    assertThat(mine).extracting(ApprovalRequest::getEntityId).containsExactlyInAnyOrder(10L, 11L);
  }
}
