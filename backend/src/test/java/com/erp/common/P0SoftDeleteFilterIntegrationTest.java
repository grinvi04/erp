 ...
 *
 * <p>{@code @SQLRestriction("deleted_at IS NULL")}이 {@code @MappedSuperclass}(BaseEntity)에 선언돼 있어 하위
 * {@code @Entity}로 상속되지 않는다 → 소프트삭제 필터가 실제 SQL에 안 붙어 목록·조회에 삭제 행이 남는다. (deleted_at 세팅형 엔티티 공통 — 여기선 HR
 * Department로 대표 박제. Department.delete()는 softDelete()로 deleted_at을 세팅한다.)
 *
...
class P0SoftDeleteFilterIntegrationTest extends AbstractIntegrationTest {
  ...
  void softDeleted_excludedFromList() {
    ...
    assertThat(ids)
        .as("소프트삭제(deleted_at)된 부서는 목록 조회에서 제외되어야 한다")
