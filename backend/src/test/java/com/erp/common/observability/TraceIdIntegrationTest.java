...
/**
 * TraceIdFilter end-to-end: permitAll 엔드포인트(/actuator/health)에서
 * X-Trace-Id 응답 헤더가 생성/전파되는지 검증. (test 프로파일 = spring.security.enabled=false)
 */
class TraceIdIntegrationTest extends AbstractIntegrationTest {
    ...
    void inboundTraceparentIsReflectedInResponseHeader() {
        ...
        assertThat(response.getHeaders().getFirst(TraceIdFilter.TRACE_ID_HEADER)).isEqualTo(expectedTraceId);
    }
}
