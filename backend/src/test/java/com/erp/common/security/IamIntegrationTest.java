...
/**
 * IAM 관리 API가 DB 인가 정본을 갱신하고, 그 결과가 AuthorizationResolver(권한 해석)에 그대로 반영되는 전체 루프를 검증한다 — 관리화면 → DB
 * → 런타임 권한.
 */
@Transactional
class IamIntegrationTest extends AbstractIntegrationTest {
  ...
  void setAccessProfile_reflectedInResolution() {
    iamService.setAccessProfile(
        "bob", new AccessProfileRequest(DataScope.DEPARTMENT, 9L, new BigDecimal("3000000")));

    UserAccessProfile profile =
        authorizationResolver.accessProfile(TEST_TENANT_ID, "bob").orElseThrow();
  ...
  void assignRole_userIdWithQuote_doesNotCorruptAuditJsonb() {
    ...
    RoleResponse role =
        iamService.createRole(new RoleCreateRequest("Q", "q", null, Set.of(Permission.CRM_READ)));
