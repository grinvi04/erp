...
class LeadControllerTest {
    ...
    private LeadResponse buildResponse() {
        return new LeadResponse(1L, "홍", "길동", "테스트회사", "대리",
                "hong@test.com", "010-0000-0000", "WEB", LeadStatus.NEW, "user-001",
                null, null, null, "메모", LocalDateTime.now(), 0L);
    }
    ...
    void findById_existingLead_returnsOk() throws Exception {
        ...
        mockMvc.perform(get("/api/crm/leads/1"))
                .andExpect(status().isOk())
    ...
    void findById_notFound_returns404() throws Exception {
        given(leadService.findById(99L))
                .willThrow(new ErpException(ErrorCode.LEAD_NOT_FOUND));

        mockMvc.perform(get("/api/crm/leads/99"))
    ...
    void update_validRequest_returnsOk() throws Exception {
        ...
        LeadUpdateRequest req = new LeadUpdateRequest("홍", "길동", "테스트회사",
                null, null, null, "WEB", "user-001", "메모", 0L);

        mockMvc.perform(put("/api/crm/leads/1")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void update_missingVersion_returns400() throws Exception {
        LeadUpdateRequest req = new LeadUpdateRequest("홍", "길동", "테스트회사",
                null, null, null, "WEB", "user-001", "메모", null);
    ...
    void convert_validRequest_returnsOk() throws Exception {
        LeadResponse converted = new LeadResponse(1L, "홍", "길동", "테스트회사", "대리",
                "hong@test.com", "010-0000-0000", "WEB", LeadStatus.CONVERTED, "user-001",
                10L, 20L, LocalDateTime.now(), "메모", LocalDateTime.now(), 1L);
        given(leadService.convert(eq(1L), any())).willReturn(converted);

        mockMvc.perform(post("/api/crm/leads/1/convert")
                        .contentType(MediaType.APPLICATION_JSON)
