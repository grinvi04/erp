...
class ContactControllerTest {
  ...
  private ContactCreateRequest validCreate() {
    return new ContactCreateRequest(
        1L, "홍", "길동", "대리", "영업팀", "hong@test.com", "02-000-0000", "010-0000-0000", true);
  }
  ...
  void findByAccount_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/crm/contacts").param("accountId", "1"))
  ...
  void findById_existingContact_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/crm/contacts/1"))
        .andExpect(status().isOk())
  ...
  void findById_notFound_returns404() throws Exception {
    given(contactService.findById(99L)).willThrow(new ErpException(ErrorCode.CONTACT_NOT_FOUND));

    mockMvc
        .perform(get("/api/crm/contacts/99"))
  ...
  void create_blankLastName_returns400() throws Exception {
    ContactCreateRequest invalid =
        new ContactCreateRequest(1L, "", "길동", null, null, null, null, null, false);
  ...
  void create_missingAccountId_returns400() throws Exception {
    ContactCreateRequest invalid =
        new ContactCreateRequest(null, "홍", "길동", null, null, null, null, null, false);
  ...
  void create_invalidEmail_returns400() throws Exception {
    ContactCreateRequest invalid =
        new ContactCreateRequest(1L, "홍", "길동", null, null, "not-an-email", null, null, false);
  ...
  void delete_existingContact_returns204() throws Exception {
    mockMvc.perform(delete("/api/crm/contacts/1")).andExpect(status().isNoContent());
  }

  @Test
  void create_noPermission_returns403() throws Exception {
    given(contactService.create(any())).willThrow(new ErpException(ErrorCode.FORBIDDEN));
  ...
  void findById_noPermission_returns403() throws Exception {
    given(contactService.findById(1L)).willThrow(new ErpException(ErrorCode.FORBIDDEN));

    mockMvc.perform(get("/api/crm/contacts/1")).andExpect(status().isForbidden());
  }

  @Test
  void delete_noPermission_returns403() throws Exception {
    willThrow(new ErpException(ErrorCode.FORBIDDEN)).given(contactService).delete(1L);

    mockMvc.perform(delete("/api/crm/contacts/1")).andExpect(status().isForbidden());
  }
}
