...
class SalesTeamControllerTest {
  ...
  void list_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/crm/sales-teams"))
  ...
  void get_existingTeam_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/crm/sales-teams/1"))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            post("/api/crm/sales-teams")
                ...
                .content(
                    objectMapper.writeValueAsString(new SalesTeamCreateRequest("T-001", "영업1팀"))))
  ...
  void create_blankCode_returns400() throws Exception {
    mockMvc
        .perform(
            post("/api/crm/sales-teams")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new SalesTeamCreateRequest("", "영업1팀"))))
  ...
  void create_blankName_returns400() throws Exception {
    mockMvc
        .perform(
            post("/api/crm/sales-teams")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new SalesTeamCreateRequest("T-001", ""))))
        .andExpect(status().isBadRequest());
  }
  ...
  void update_validRequest_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/crm/sales-teams/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new SalesTeamUpdateRequest("영업1팀", 0L))))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.id").value(1));
  }
  ...
  void update_missingVersion_returns400() throws Exception {
    mockMvc
        .perform(
            put("/api/crm/sales-teams/1")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new SalesTeamUpdateRequest("영업1팀", null))))
        .andExpect(status().isBadRequest());
  }

  @Test
  void delete_existingTeam_returns204() throws Exception {
    mockMvc.perform(delete("/api/crm/sales-teams/1")).andExpect(status().isNoContent());
  }
  ...
  void addMember_validRequest_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/crm/sales-teams/1/members")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new TeamMemberRequest("user-001"))))
        .andExpect(status().isOk())
  ...
  void addMember_blankUserId_returns400() throws Exception {
    mockMvc
        .perform(
            post("/api/crm/sales-teams/1/members")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(new TeamMemberRequest(""))))
        .andExpect(status().isBadRequest());
  }
  ...
  void removeMember_validRequest_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/crm/sales-teams/1/members/user-001"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.id").value(1));
  }
  ...
  void list_noPermission_returns403() throws Exception {
    given(salesTeamService.listTeams()).willThrow(new ErpException(ErrorCode.FORBIDDEN));

    mockMvc.perform(get("/api/crm/sales-teams")).andExpect(status().isForbidden());
  }

  @Test
  void delete_noPermission_returns403() throws Exception {
    willThrow(new ErpException(ErrorCode.FORBIDDEN)).given(salesTeamService).deleteTeam(1L);

    mockMvc.perform(delete("/api/crm/sales-teams/1")).andExpect(status().isForbidden());
  }
}
