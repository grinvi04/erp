...
class ActivityControllerTest {
  ...
  private ActivityCreateRequest validCreate() {
    return new ActivityCreateRequest(
        ActivityType.CALL, "신규 고객 전화", 1L, 2L, 3L, LocalDateTime.of(2026, 7, 1, 10, 0), "통화 메모");
  }

  @Test
  void search_returnsOk() throws Exception {
    PageResponse<ActivityResponse> page =
        new PageResponse<>(List.of(buildResponse(ActivityStatus.OPEN)), 0, 20, 1, 1, true, true);
    given(activityService.search(any(), any(), any(), any(), any())).willReturn(page);

    mockMvc
        .perform(get("/api/crm/activities").param("activityType", "CALL").param("status", "OPEN"))
  ...
  void findById_existingActivity_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/crm/activities/1"))
        .andExpect(status().isOk())
  ...
  void findById_notFound_returns404() throws Exception {
    given(activityService.findById(99L)).willThrow(new ErpException(ErrorCode.ACTIVITY_NOT_FOUND));

    mockMvc
        .perform(get("/api/crm/activities/99"))
  ...
  void create_missingActivityType_returns400() throws Exception {
    ActivityCreateRequest invalid =
        new ActivityCreateRequest(null, "신규 고객 전화", 1L, 2L, 3L, null, null);
  ...
  void create_blankSubject_returns400() throws Exception {
    ActivityCreateRequest invalid =
        new ActivityCreateRequest(ActivityType.CALL, "", 1L, 2L, 3L, null, null);
  ...
  void complete_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(post("/api/crm/activities/1/complete"))
        .andExpect(status().isOk())
  ...
  void cancel_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(post("/api/crm/activities/1/cancel"))
        .andExpect(status().isOk())
  ...
  void complete_notFound_returns404() throws Exception {
    given(activityService.complete(99L)).willThrow(new ErpException(ErrorCode.ACTIVITY_NOT_FOUND));

    mockMvc.perform(post("/api/crm/activities/99/complete")).andExpect(status().isNotFound());
  }

  @Test
  void delete_existingActivity_returns204() throws Exception {
    mockMvc.perform(delete("/api/crm/activities/1")).andExpect(status().isNoContent());
  }

  @Test
  void create_noPermission_returns403() throws Exception {
    given(activityService.create(any())).willThrow(new ErpException(ErrorCode.FORBIDDEN));
  ...
  void complete_noPermission_returns403() throws Exception {
    given(activityService.complete(1L)).willThrow(new ErpException(ErrorCode.FORBIDDEN));

    mockMvc.perform(post("/api/crm/activities/1/complete")).andExpect(status().isForbidden());
  }

  @Test
  void delete_noPermission_returns403() throws Exception {
    willThrow(new ErpException(ErrorCode.FORBIDDEN)).given(activityService).delete(1L);

    mockMvc.perform(delete("/api/crm/activities/1")).andExpect(status().isForbidden());
  }
}
