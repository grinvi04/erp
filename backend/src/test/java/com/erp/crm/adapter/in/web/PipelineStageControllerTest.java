...
class PipelineStageControllerTest {
  ...
  void findAll_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/crm/pipeline-stages"))
  ...
  void findById_existingStage_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(get("/api/crm/pipeline-stages/1"))
        .andExpect(status().isOk())
  ...
  void findById_notFound_returns404() throws Exception {
    ...
    mockMvc
        .perform(get("/api/crm/pipeline-stages/99"))
  ...
  void create_blankName_returns400() throws Exception {
    PipelineStageCreateRequest invalid = new PipelineStageCreateRequest("", 1, 20, false, false);
  ...
  void create_invalidStageOrder_returns400() throws Exception {
    PipelineStageCreateRequest invalid = new PipelineStageCreateRequest("탐색", 0, 20, false, false);
  ...
  void create_probabilityOutOfRange_returns400() throws Exception {
    PipelineStageCreateRequest invalid = new PipelineStageCreateRequest("탐색", 1, 150, false, false);
  ...
  void update_notFound_returns404() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/crm/pipeline-stages/99")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(validUpdate())))
        .andExpect(status().isNotFound());
  }
  ...
  void delete_existingStage_returns204() throws Exception {
    mockMvc.perform(delete("/api/crm/pipeline-stages/1")).andExpect(status().isNoContent());
  }

  @Test
  void create_noPermission_returns403() throws Exception {
    given(stageService.create(any())).willThrow(new ErpException(ErrorCode.FORBIDDEN));
  ...
  void findById_noPermission_returns403() throws Exception {
    given(stageService.findById(1L)).willThrow(new ErpException(ErrorCode.FORBIDDEN));

    mockMvc.perform(get("/api/crm/pipeline-stages/1")).andExpect(status().isForbidden());
  }

  @Test
  void delete_noPermission_returns403() throws Exception {
    willThrow(new ErpException(ErrorCode.FORBIDDEN)).given(stageService).delete(1L);

    mockMvc.perform(delete("/api/crm/pipeline-stages/1")).andExpect(status().isForbidden());
  }
}
