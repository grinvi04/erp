...
class PipelineStageServiceTest {
  ...
  void findAll_returnsOrderedStages() {
    given(stageRepository.findAllByOrderByStageOrderAsc()).willReturn(List.of(buildStage()));
  ...
  void update_versionMismatch_throwsOptimisticLockConflict() {
    ...
    assertThrows(ObjectOptimisticLockingFailureException.class, () -> stageService.update(1L, req));
  }
