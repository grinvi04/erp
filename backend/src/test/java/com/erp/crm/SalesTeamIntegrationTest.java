...
class SalesTeamIntegrationTest extends AbstractIntegrationTest {
  ...
  void create_update_delete_roundTrip() {
    ...
    SalesTeamResponse updated =
        salesTeamService.updateTeam(created.id(), "강남2팀", created.version());
    assertThat(updated.name()).isEqualTo("강남2팀");

    assertThat(salesTeamService.listTeams()).extracting(SalesTeamResponse::code).contains("ST-01");
  ...
  void addMember_removeMember_persisted() {
    ...
    assertThat(salesTeamService.getTeam(team.id()).memberUserIds()).containsExactly("user-002");
  }
}
