...
class PipelineDistributionIntegrationTest extends AbstractIntegrationTest {
  ...
  void pipelineDistribution_separatesAmountsByCurrencyAndPreservesEmptyStages() {
    ...
    PipelineStage stageA =
        pipelineStageRepository.save(PipelineStage.of("Prospecting", 1, 10, false, false));
    // Stage B: order=2, zero opportunities
    PipelineStage stageB =
        pipelineStageRepository.save(PipelineStage.of("Closed Won", 2, 100, true, false));
    ...
    PipelineDistributionResponse rowA =
        result.stream().filter(r -> r.stageId().equals(stageA.getId())).findFirst().orElseThrow();
    ...
    PipelineDistributionResponse rowB =
