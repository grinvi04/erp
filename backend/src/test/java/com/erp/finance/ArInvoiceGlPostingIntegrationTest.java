...
/**
 * AR 전표 승인 → GL 자동 분개(DRAFT) 전체 흐름 검증. 실무 분개(AP와 반전): (차) 외상매출금[고객 통제계정] / (대) 매출[라인 계정] —
 * 균형·DRAFT·역참조 확인. DEBIT 측이 외상매출금 계정임을 검증한다.
 */
@Transactional
class ArInvoiceGlPostingIntegrationTest extends AbstractIntegrationTest {
    ...
    var created =
        arInvoiceService.create(
            new ArInvoiceCreateRequest(
                ...
                null,
                List.of(
                    new ArInvoiceLineRequest(
                        revenueAccountId, new BigDecimal("100000"), "매출 발생"))));
  ...
  void pay_withCashAccount_createsBalancedReceiptDraftJournalEntry() {
    ...
    arInvoiceService.pay(
        created.id(),
        new ArInvoicePayRequest(new BigDecimal("100000"), cash.getId(), LocalDate.of(2025, 1, 20)));

    JournalEntry payJe =
        journalEntryRepository
            .findByReferenceTypeAndReferenceId(ReferenceTypes.AR_PAYMENT, created.id())
            .orElseThrow();
