...
/**
 * 실현 환차손익 — 외화 AP 지급·AR 수금 시 결제환율≠인보이스환율 차액을 환차손/환차이익 라인으로
 * 자동 분개하는 전체 흐름 검증(기준통화 KRW). 통제계정은 인보이스환율로 청산되고 차대변이 균형한다.
 * 폴백(기준통화·환율/계정 부재)은 기존 원통화 2라인을 유지한다.
 */
@Transactional
class FxGainLossPostingIntegrationTest extends AbstractIntegrationTest {
    ...
    void setUp() {
        ...
        customer.assignReceivablesAccount(accountRepository.findById(receivablesAccountId).orElseThrow());
    ...
    private Long createApprovedApInvoice(String no, String currency, LocalDate invoiceDate, BigDecimal amount) {
        authenticate("creator", BigDecimal.ZERO, "finance:write");
        var created = apInvoiceService.create(new ApInvoiceCreateRequest(no, vendorId, invoiceDate,
                invoiceDate.plusMonths(1), amount, currency, null,
    ...
    private Long createApprovedArInvoice(String no, String currency, LocalDate invoiceDate, BigDecimal amount) {
        authenticate("creator", BigDecimal.ZERO, "finance:write");
        var created = arInvoiceService.create(new ArInvoiceCreateRequest(no, customerId, invoiceDate,
                invoiceDate.plusMonths(1), amount, currency, null,
    ...
    private BigDecimal debitOn(JournalEntry je, Long accountId) {
        return je.getLines().stream().filter(l -> l.getAccount().getId().equals(accountId))
    ...
    void fallback_noPaymentRate_keepsForeignCurrencyTwoLines() {
        ...
        Long inv = createApprovedApInvoice("AP-NOPAYRATE", "USD", LocalDate.of(2025, 1, 15), new BigDecimal("100"));
        configureFxAccounts();
