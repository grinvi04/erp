...
/**
 * 기준통화 변경 가드(PR) 통합 검증 — base_amount 스냅샷이 존재하는 finance 거래(ap_invoice)가 있으면
 * 기준통화 변경을 거부(409 BASE_CURRENCY_CHANGE_NOT_ALLOWED)하고 기존 설정이 불변임을, 스냅샷이 없으면
 * 변경이 허용됨을 실제 DB·@TenantId 필터로 확인한다. 동일 값 PUT(no-op)도 허용을 확인.
 */
@Transactional
class FxBaseCurrencyGuardIntegrationTest extends AbstractIntegrationTest {
    ...
    private void seedPricedApInvoice() {
        ...
        ApInvoice inv = ApInvoice.create("AP-GUARD", vendor, LocalDate.of(2025, 1, 1),
            LocalDate.of(2025, 1, 31), new BigDecimal("100"), "USD", null);
