...
class ApInvoiceControllerTest {
    ...
    private ApInvoiceResponse sampleResponse() {
        return new ApInvoiceResponse(1L, "INV-001", 1L, "공급사",
            LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 31),
            new BigDecimal("100000"), BigDecimal.ZERO, new BigDecimal("100000"),
            "KRW", ApInvoiceStatus.DRAFT, null, null, null);
    }
    ...
    void pay_invalidAmount_returns400() throws Exception {
        mockMvc.perform(post("/api/finance/invoices/1/pay")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"amount\":0}"))
    ...
    void cancel_alreadyProcessed_returns409() throws Exception {
        given(apInvoiceService.cancel(1L)).willThrow(new ErpException(ErrorCode.INVOICE_ALREADY_PROCESSED));

        mockMvc.perform(post("/api/finance/invoices/1/cancel"))
