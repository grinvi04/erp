...
class FiscalYearControllerTest {
    ...
    void findAll_returnsOk() throws Exception {
        ...
        mockMvc.perform(get("/api/finance/fiscal-years"))
    ...
    void create_validRequest_returns201() throws Exception {
        ...
        FiscalYearResponse response = new FiscalYearResponse(1L, 2025,
            LocalDate.of(2025, 1, 1), LocalDate.of(2025, 12, 31), FiscalYearStatus.OPEN);
        given(fiscalYearService.create(any())).willReturn(response);

        mockMvc.perform(post("/api/finance/fiscal-years")
    ...
    void close_notFound_returns404() throws Exception {
        willThrow(new ErpException(ErrorCode.FISCAL_YEAR_NOT_FOUND))
            .given(fiscalYearService).close(99L);

        mockMvc.perform(post("/api/finance/fiscal-years/99/close"))
