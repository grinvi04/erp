...
class JournalEntryControllerTest {
  ...
  void findByFiscalPeriod_returnsOkWithPage() throws Exception {
    ...
    mockMvc
        .perform(get("/api/finance/journal-entries?fiscalPeriodId=1"))
  ...
  void submit_notFound_returns404() throws Exception {
    ...
    mockMvc
        .perform(post("/api/finance/journal-entries/99/submit"))
  ...
  void approve_returnsOk() throws Exception {
    ...
    mockMvc
        .perform(post("/api/finance/journal-entries/1/approve"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true));
  }
}
