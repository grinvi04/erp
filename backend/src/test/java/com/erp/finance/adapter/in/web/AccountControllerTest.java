...
class AccountControllerTest {
  ...
  void findAll_returnsOkWithList() throws Exception {
    AccountResponse response =
        new AccountResponse(
            1L, "1100", "현금", AccountType.ASSET, NormalBalance.DEBIT, null, null, false, true, 0L);
    given(accountService.findAll()).willReturn(List.of(response));

    mockMvc
        .perform(get("/api/finance/accounts"))
                ...
                .content(
                    objectMapper.writeValueAsString(
                        new AccountCreateRequest(
                            "1100", "현금", AccountType.ASSET, NormalBalance.DEBIT, null, false))))
  ...
  void create_duplicateCode_returns409() throws Exception {
    given(accountService.create(any()))
        .willThrow(new ErpException(ErrorCode.ACCOUNT_CODE_DUPLICATE));
