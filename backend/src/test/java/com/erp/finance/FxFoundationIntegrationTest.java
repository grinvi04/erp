...
/**
 * FX 기반(PR1) 통합 검증 — 기준통화 GET/PUT·환율 등록(중복 거부)·권한403(FINANCE_SETTING_WRITE 부재)· 환산(최신
 * rate≤date·기준통화 그대로·부재 시 CURRENCY_RATE_NOT_FOUND)을 실제 DB·SecurityContext로 확인한다. 권한
 * 검사(authority)·@TenantId·repo 쿼리는 Mockito로는 검증 안 되므로 전체 컨텍스트로 보강한다. 외부 provider는 @MockBean으로 격리(CI
 * 네트워크 의존 금지).
 */
@Transactional
class FxFoundationIntegrationTest extends AbstractIntegrationTest {
  ...
  void convert_baseCurrency_returnsAmountUnchanged() {
    ...
    assertThat(
            currencyConverter.convert(new BigDecimal("1000000"), "KRW", LocalDate.of(2024, 6, 1)))
