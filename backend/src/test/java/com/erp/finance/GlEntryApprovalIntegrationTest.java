...
/**
 * GL 전표 전기 결재선 전체 흐름 검증: DRAFT→상신(PENDING_APPROVAL)→다른 사용자 승인→POSTED. 직무분리(작성자 본인 결재 차단), 전결권 미보유
 * 차단, 직접 전기 차단, 결재함 노출을 함께 검증한다.
 */
@Transactional
class GlEntryApprovalIntegrationTest extends AbstractIntegrationTest {
  ...
  void approveBySelfCreator_throwsApproverNotAuthorized() {
    ...
    ErpException ex =
        assertThrows(ErpException.class, () -> journalEntryService.approve(created.id()));
  ...
  void pendingEntry_appearsInApproverInbox() {
    ...
    assertThat(inbox)
        .anyMatch(
            s ->
                ReferenceTypes.GL_ENTRY.equals(s.entityType())
