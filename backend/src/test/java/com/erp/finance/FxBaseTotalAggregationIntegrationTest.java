...
/**
 * AC-12: 요약 기준통화 합계 집계가 base_amount 산정분만 합산하고(미산정 null 행 제외),
 * 기존 통화별 분리 합계는 영향받지 않는지(회귀) 검증한다.
 * base_amount는 거래 시점 스냅샷(PR2)이므로 합계는 단순 SUM이며, 여기서는 산정분은 applyBaseSnapshot으로,
 * 미산정 행은 스냅샷 미적용으로 만든다.
 */
@Transactional
class FxBaseTotalAggregationIntegrationTest extends AbstractIntegrationTest {
    ...
    void apInvoice_unpaidBaseTotal_usesOutstandingBalance_andStaysConsistentWithCurrencySplit() {
        Vendor vendor = vendorRepository.save(Vendor.of(
                "V-FX", "FX공급사", "000-00-11111", "홍길동", "v@test.com", "010-0000-0000", 30));
        // 부분지급(KRW, rate 1): 전액 1,000,000 스냅샷이지만 400,000 지급 → 미지급잔액 600,000
        apInvoiceRepository.save(pricedPartiallyPaid(vendor, "AP-KRW", "1000000", "KRW", "1000000", "1", "400000"));
        ...
        BigDecimal pricedSplitInBase = amountOf(split, "KRW").multiply(BigDecimal.ONE)
    ...
    void apInvoice_unpaidBaseTotal_nullWhenNoPricedRows() {
        Vendor vendor = vendorRepository.save(Vendor.of(
                "V-NP", "미산정공급사", "000-00-22222", "홍길동", "np@test.com", "010-0000-0000", 30));
    ...
    void opportunity_openBaseTotal_sumsPricedRowsOnly_andCurrencySplitUnaffected() {
        ...
        Account account = crmAccountRepository.save(Account.of(
                "ACC-FX", "FX고객사", null, null, null, null, null, null, null, AccountType.CUSTOMER, "owner"));

        opportunityRepository.save(pricedOpp(account, stage, "OPP-KRW", "5000000", "KRW", "5000000", "1"));
        opportunityRepository.save(pricedOpp(account, stage, "OPP-USD", "1000", "USD", "1300000", "1300"));
        ...
        // 기준통화 합계 = 산정분만(JPY 제외)
        assertThat(opportunityRepository.sumOpenBaseTotal(allScope, noOwners)).isEqualByComparingTo("6300000");
    ...
    private ApInvoice priced(Vendor vendor, String no, String amount, String currency, String base, String rate) {
    ...
    private ApInvoice pricedPartiallyPaid(Vendor vendor, String no, String total, String currency,
                                          String base, String rate, String paid) {
    ...
    private ApInvoice unpriced(Vendor vendor, String no, String amount, String currency) {
        return ApInvoice.create(no, vendor, LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 31),
                new BigDecimal(amount), currency, null);
    }

    private Opportunity pricedOpp(Account account, PipelineStage stage, String name, String amount,
                                  String currency, String base, String rate) {
    ...
    private Opportunity unpricedOpp(Account account, PipelineStage stage, String name, String amount, String currency) {
        return Opportunity.of(account, name, stage, new BigDecimal(amount), currency,
                LocalDate.of(2026, 12, 31), 10, "owner", null, null);
    }

    private BigDecimal amountOf(List<CurrencyAmount> list, String currency) {
        return list.stream().filter(c -> c.currency().equals(currency)).findFirst().orElseThrow().amount();
    }
}
