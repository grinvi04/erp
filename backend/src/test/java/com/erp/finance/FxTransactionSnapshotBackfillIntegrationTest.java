...
/**
 * AC-10: V2007(finance)·V4004(crm) 백필 마이그레이션이 기존 거래에 base_amount를 산정하는지 검증한다.
 * 행 통화 == 테넌트 기준통화면 base=원액·rate=1, 비기준 통화는 null(미산정) 유지.
 * 컬럼 추가는 IF NOT EXISTS, UPDATE는 base_amount IS NULL 가드로 멱등하므로 마이그레이션 SQL을
 * 시드 데이터에 재적용해 백필 로직을 직접 검증한다(MovementApproveBackfill 패턴).
 */
@Transactional
class FxTransactionSnapshotBackfillIntegrationTest extends AbstractIntegrationTest {
    ...
    void backfill_apInvoice_setsBaseForBaseCurrencyRowsAndLeavesForeignNull() throws Exception {
        ...
        Long vendorId = jdbcTemplate.queryForObject(
                ...
                Long.class, TEST_TENANT_ID, "V-FX", "FX공급사");
        ...
        jdbcTemplate.update("UPDATE finance.ap_invoice SET base_amount = NULL, exchange_rate = NULL WHERE id IN (?, ?)",
                krwId, usdId);
    ...
    private Long insertApInvoice(Long vendorId, String no, BigDecimal amount, String currency) {
        return jdbcTemplate.queryForObject(
                "INSERT INTO finance.ap_invoice (tenant_id, invoice_no, vendor_id, invoice_date, due_date, total_amount, currency) "
                        + "VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id",
                Long.class, TEST_TENANT_ID, no, vendorId,
                LocalDate.of(2025, 1, 1), LocalDate.of(2025, 1, 31), amount, currency);
    }

    private Long insertOpportunity(Long accountId, Long stageId, String name, BigDecimal amount, String currency) {
        return jdbcTemplate.queryForObject(
                "INSERT INTO crm.opportunity (tenant_id, account_id, name, stage_id, amount, currency, owner_id) "
                        + "VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id",
                Long.class, TEST_TENANT_ID, accountId, name, stageId, amount, currency, "owner");
    }

    private BigDecimal baseAmount(String table, Long id) {
        return jdbcTemplate.queryForObject("SELECT base_amount FROM " + table + " WHERE id = ?", BigDecimal.class, id);
    }

    private BigDecimal rate(String table, Long id) {
        return jdbcTemplate.queryForObject("SELECT exchange_rate FROM " + table + " WHERE id = ?", BigDecimal.class, id);
    }
}
