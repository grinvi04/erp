...
/**
 * AP 전표 승인 → GL 자동 분개(DRAFT) 전체 흐름 검증. 실무 분개: (차) 비용[라인 계정] / (대) 외상매입금[공급업체 통제계정] — 균형·DRAFT·역참조
 * 확인.
 */
@Transactional
class ApInvoiceGlPostingIntegrationTest extends AbstractIntegrationTest {
            ...
            new ApInvoiceCreateRequest(
                ...
                List.of(
                    new ApInvoiceLineRequest(
                        expenseAccountId, new BigDecimal("100000"), "소모품 구매"))));
  ...
  void pay_withCashAccount_createsBalancedPaymentDraftJournalEntry() {
    ...
    var created =
        apInvoiceService.create(
            new ApInvoiceCreateRequest(
                ...
                null,
                List.of(
                    new ApInvoiceLineRequest(expenseAccountId, new BigDecimal("100000"), "구매"))));
    apInvoiceService.submit(created.id());
    authenticate("approver", new BigDecimal("1000000"), "finance:invoice:approve");
    ...
    apInvoiceService.pay(
        created.id(),
        new ApInvoicePayRequest(new BigDecimal("100000"), cash.getId(), LocalDate.of(2025, 1, 20)));

    JournalEntry payJe =
        journalEntryRepository
            .findByReferenceTypeAndReferenceId(ReferenceTypes.AP_PAYMENT, created.id())
            .orElseThrow();
