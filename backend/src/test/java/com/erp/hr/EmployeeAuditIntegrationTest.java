...
/**
 * 직원 생명주기 변경(채용·발령·퇴직 등)이 감사 로그에 누가·무엇을·어떤 이벤트로 기록되는지
 * 실 컨텍스트로 검증한다 — 인사 데이터 변경은 노동/컴플라이언스 감사 대상.
 */
@Transactional
class EmployeeAuditIntegrationTest extends AbstractIntegrationTest {
    ...
    private EmployeeResponse createEmployee() {
        return employeeService.create(new EmployeeCreateRequest(
                "EMP-100", "김", "직원", LocalDate.of(1990, 1, 1), PersonalInfo.Gender.MALE,
    ...
    void create_writesHireAuditLog() {
        ...
        List<AuditLog> logs = auditLogRepository.search(
                TEST_TENANT_ID, "EMPLOYEE", created.id(), "hr-admin", PageRequest.of(0, 10)).getContent();
    ...
    void terminate_writesTerminateAuditLog() {
        ...
        employeeService.terminate(created.id(), new EmployeeTerminateRequest(LocalDate.of(2026, 1, 31)));
        ...
        assertThat(logs).anyMatch(l -> l.getAction() == AuditLog.AuditAction.UPDATE
                && l.getAfterData().contains("TERMINATE"));
        assertThat(logs).anyMatch(l -> l.getAction() == AuditLog.AuditAction.CREATE
                && l.getAfterData().contains("HIRE"));
    }
}
