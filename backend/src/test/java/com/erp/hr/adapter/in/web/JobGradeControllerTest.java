...
class JobGradeControllerTest {
  ...
  @MockBean private JobGradeService jobGradeService;
  ...
  void create_validRequest_returns201() throws Exception {
    JobGradeCreateRequest request =
        new JobGradeCreateRequest(
            "G3", "3급", 3, new BigDecimal("3000000"), new BigDecimal("5000000"));
    JobGradeResponse response =
        new JobGradeResponse(
            1L, "G3", "3급", 3, new BigDecimal("3000000"), new BigDecimal("5000000"), 0L);
    given(jobGradeService.create(any())).willReturn(response);

    mockMvc
        .perform(
            post("/api/hr/job-grades")
  ...
  void delete_gradeInUse_returns409() throws Exception {
    willThrow(new ErpException(ErrorCode.JOB_GRADE_IN_USE)).given(jobGradeService).delete(1L);

    mockMvc
        .perform(delete("/api/hr/job-grades/{id}", 1L))
        .andExpect(status().isConflict())
  ...
  void delete_existing_returns204() throws Exception {
    mockMvc.perform(delete("/api/hr/job-grades/{id}", 1L)).andExpect(status().isNoContent());
  }
}
