...
class DepartmentControllerTest {
  ...
  void findAll_returnsOkWithList() throws Exception {
    ...
    mockMvc
        .perform(get("/api/hr/departments"))
  ...
  void create_validRequest_returns201() throws Exception {
    ...
    DepartmentResponse response =
        new DepartmentResponse(1L, "DEV", "개발팀", null, 0, 0, null, true, 0L);
  ...
  void delete_withChildren_returns409() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/hr/departments/{id}", 1L))
        .andExpect(status().isConflict())
  ...
  void delete_existing_returns204() throws Exception {
    mockMvc.perform(delete("/api/hr/departments/{id}", 1L)).andExpect(status().isNoContent());
  }
}
