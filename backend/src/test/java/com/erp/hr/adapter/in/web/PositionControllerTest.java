...
class PositionControllerTest {
  ...
  void delete_positionInUse_returns409() throws Exception {
    willThrow(new ErpException(ErrorCode.POSITION_IN_USE)).given(positionService).delete(1L);

    mockMvc
        .perform(delete("/api/hr/positions/{id}", 1L))
        .andExpect(status().isConflict())
  ...
  void delete_existing_returns204() throws Exception {
    mockMvc.perform(delete("/api/hr/positions/{id}", 1L)).andExpect(status().isNoContent());
  }
}
