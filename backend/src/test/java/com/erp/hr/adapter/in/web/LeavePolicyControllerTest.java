...
class LeavePolicyControllerTest {
  ...
  void findAll_returnsOkWithList() throws Exception {
    LeavePolicyResponse response =
        new LeavePolicyResponse(1L, "ANNUAL", "연차", LeavePolicy.LeaveType.ANNUAL, 15, 5, true, 1);
    given(leavePolicyService.findAll()).willReturn(List.of(response));

    mockMvc
        .perform(get("/api/hr/leave-policies"))
  ...
  void create_validRequest_returns201() throws Exception {
    LeavePolicyResponse response =
        new LeavePolicyResponse(1L, "SICK", "병가", LeavePolicy.LeaveType.SICK, 10, 0, false, 0);
    given(leavePolicyService.create(any())).willReturn(response);

    LeavePolicyCreateRequest request =
        new LeavePolicyCreateRequest("SICK", "병가", LeavePolicy.LeaveType.SICK, 10, 0, false, 0);
  ...
  void create_duplicateCode_returns409() throws Exception {
    ...
    LeavePolicyCreateRequest request =
        new LeavePolicyCreateRequest("ANNUAL", "연차", LeavePolicy.LeaveType.ANNUAL, 15, 5, true, 1);
  ...
  void delete_existing_returns204() throws Exception {
    mockMvc.perform(delete("/api/hr/leave-policies/1")).andExpect(status().isNoContent());
  }

  @Test
  void delete_notFound_returns404() throws Exception {
    willThrow(new ErpException(ErrorCode.RESOURCE_NOT_FOUND)).given(leavePolicyService).delete(99L);

    mockMvc
        .perform(delete("/api/hr/leave-policies/99"))
