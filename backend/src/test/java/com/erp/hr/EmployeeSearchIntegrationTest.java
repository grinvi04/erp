...
class EmployeeSearchIntegrationTest extends AbstractIntegrationTest {
  ...
  private void save(
      String empNo, String lastName, String firstName, Department dept, Position pos) {
    PersonalInfo info =
        new PersonalInfo(
            lastName,
            firstName,
  ...
  private void authenticate(String sub) {
    ...
    accessProfileRepository.save(
        UserAccessProfile.of(TEST_TENANT_ID, sub, DataScope.ALL, null, null));
  }

  private List<String> searchEmpNos(String keyword) {
    return employeeService.findAll(null, null, keyword, PageRequest.of(0, 50)).getContent().stream()
        .map(EmployeeResponse::employeeNo)
        .toList();
  }
