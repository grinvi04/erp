...
class HrAnalyticsIntegrationTest extends AbstractIntegrationTest {
  ...
  void setUpData() {
    ...
    empA =
        save(
            ...
            false);
    save(
        "EMP-B", "user-b", deptB, posMgr, EmploymentType.CONTRACT, LocalDate.of(2026, 3, 5), false);
    empC =
        save(
            "EMP-C",
            "user-c",
  ...
  private Employee save(
    ...
    PersonalInfo info =
        new PersonalInfo(
            "성", empNo, LocalDate.of(1990, 1, 1), PersonalInfo.Gender.MALE, null, null, null);
    Employee e =
        Employee.create(
            empNo,
            info,
  ...
  void statusDistribution_countsByStatus_allEnumValuesPresent() {
    ...
    assertThat(rows)
        .extracting(EmployeeStatusCountResponse::status)
  ...
  void byPosition_countsActive_preservesEmptyPosition_orderedByLevel() {
    ...
    assertThat(rows)
        .extracting(PositionHeadcountResponse::positionId)
        .containsExactly(posMgr.getId(), posEng.getId(), posUnused.getId()); // level_order 순
    assertThat(posCount(rows, posEng.getId()))
        .isEqualTo(2L); // empA, empC ACTIVE (empD terminated 제외)
  ...
  void byEmploymentType_countsAll_allEnumValuesPresent() {
    ...
    assertThat(rows)
        .extracting(EmploymentTypeCountResponse::employmentType)
  ...
  void leavesByType_countsApprovedOnly_withDaySum() {
    ...
    assertThat(rows)
        .extracting(LeaveTypeStatResponse::leaveType)
  ...
  void departmentScope_excludesOtherDepartments() {
    ...
    assertThat(byDept)
        .extracting(DepartmentHeadcountResponse::departmentId)
  ...
  private void seedLeaves() {
    ...
    LeavePolicy sick =
        leavePolicyRepository.save(LeavePolicy.of("SCK", "병가", LeaveType.SICK, 10, 0, true, 1));
  ...
  private void approved(Employee emp, LeavePolicy policy, BigDecimal days) {
    LeaveRequest req =
        LeaveRequest.create(
            emp, policy, LocalDate.of(2026, 4, 1), LocalDate.of(2026, 4, 2), days, "r");
    req.approve();
