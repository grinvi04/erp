...
class EmployeeDataScopeIntegrationTest extends AbstractIntegrationTest {
  ...
  private void authenticate(String sub, String dataScope, Long departmentId) {
    ...
    accessProfileRepository.save(
        UserAccessProfile.of(TEST_TENANT_ID, sub, scope, departmentId, null));
  }
  ...
  void leaveRequestFindAll_departmentScope_excludesOutOfScopeEmployees() {
    ...
    List<Long> empIds =
        leaveRequestService.findAll(PageRequest.of(0, 50)).getContent().stream()
  ...
  void findById_outOfDepartmentScope_throwsForbidden() {
    ...
    Employee empC =
        employeeRepository.findAll().stream()
            .filter(e -> e.getEmployeeNo().equals("EMP-C"))
            .findFirst()
            .orElseThrow();
    authenticate("user-a", "DEPARTMENT", deptA.getId());

    org.junit.jupiter.api.Assertions.assertThrows(
        com.erp.common.exception.ErpException.class, () -> employeeService.findById(empC.getId()));
  }
}
