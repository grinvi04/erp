...
class JobGradeServiceTest {
  ...
  @InjectMocks private JobGradeService jobGradeService;

  @Test
  void create_newCode_returnsCreatedJobGrade() {
    given(jobGradeRepository.existsByCode("G3")).willReturn(false);
    JobGrade grade =
        JobGrade.of("G3", "3급", 3, new BigDecimal("3000000"), new BigDecimal("5000000"));
    given(jobGradeRepository.save(any())).willReturn(grade);

    JobGradeResponse result =
        jobGradeService.create(
