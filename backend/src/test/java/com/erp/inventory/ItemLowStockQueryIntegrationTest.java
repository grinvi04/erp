...
class ItemLowStockQueryIntegrationTest extends AbstractIntegrationTest {
  ...
  private Item createItem(String sku, BigDecimal reorderPoint, boolean active) {
    ItemCategory cat =
        itemCategoryRepository.save(ItemCategory.of("CAT-" + sku, "Category " + sku, null));
  ...
  private Stock createStock(Item item, BigDecimal qty) {
    Warehouse wh =
        warehouseRepository.save(
            Warehouse.of("WH-" + item.getSku(), "Warehouse " + item.getSku(), null));
    Location loc =
        locationRepository.save(
            Location.of(
                wh, "LOC-" + item.getSku(), "Location " + item.getSku(), null, LocationType.BIN));
  ...
  void countLowStockItems_zeroStockIsLowStock_explicitBehavior() {
    // Verify explicitly: an item with no stock rows is treated as low stock (COALESCE 0 <=
    // reorderPoint).
    ...
    assertThat(itemRepository.countLowStockItems())
        .as(
            "Active item with no stock rows should register as low-stock via COALESCE(SUM, 0) <= reorderPoint")
        .isEqualTo(1L);
