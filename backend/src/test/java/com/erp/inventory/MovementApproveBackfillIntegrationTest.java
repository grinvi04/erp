...
/**
 * AC-14: V0006 백필 마이그레이션이 inventory:write 보유 역할에 inventory:movement:approve를 멱등하게 부여하는지 실제 마이그레이션
 * SQL을 시드 데이터에 적용해 검증한다(중복 INSERT 없음).
 */
@Transactional
class MovementApproveBackfillIntegrationTest extends AbstractIntegrationTest {
  ...
  private String migrationSql() throws Exception {
    return StreamUtils.copyToString(
        new ClassPathResource("db/migration/V0006__inventory_movement_approve_backfill.sql")
            .getInputStream(),
