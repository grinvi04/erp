...
class StockControllerTest {
  ...
  void findByItem_returnsOkWithPage() throws Exception {
    PageResponse<StockResponse> page =
        new PageResponse<>(List.of(buildStockResponse()), 0, 10, 1, 1, true, true);
    given(stockService.findByItem(eq(1L), any())).willReturn(page);

    mockMvc
        .perform(get("/api/inventory/stocks/by-item").param("itemId", "1"))
  ...
  void findByWarehouse_returnsOkWithPage() throws Exception {
    ...
    mockMvc
        .perform(get("/api/inventory/stocks/by-warehouse").param("warehouseId", "1"))
        .andExpect(status().isOk())
