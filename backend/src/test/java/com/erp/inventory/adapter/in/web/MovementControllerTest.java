...
class MovementControllerTest {
  ...
  void findAll_returnsOkWithPage() throws Exception {
    PageResponse<MovementResponse> page =
        new PageResponse<>(List.of(buildResponse(MovementStatus.DRAFT)), 0, 10, 1, 1, true, true);
  ...
  void confirm_validId_returnsConfirmed() throws Exception {
    ...
    mockMvc
        .perform(post("/api/inventory/movements/1/confirm"))
        .andExpect(status().isOk())
  ...
  void confirm_insufficientStock_returns409() throws Exception {
    ...
    mockMvc.perform(post("/api/inventory/movements/1/confirm")).andExpect(status().isConflict());
  }
  ...
  void cancel_validId_returnsCancelled() throws Exception {
    ...
    mockMvc
        .perform(post("/api/inventory/movements/1/cancel"))
        .andExpect(status().isOk())
