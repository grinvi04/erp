...
class InventoryAnalyticsIntegrationTest extends AbstractIntegrationTest {
  ...
  private Stock stock(
      Item item, Location location, String lotNo, BigDecimal qty, BigDecimal unitCost) {
  ...
  private Movement movement(
      MovementType type, LocalDate date, boolean confirm, BigDecimal... lineQtys) {
    ...
    for (BigDecimal qty : lineQtys) {
      movementLineRepository.save(
          MovementLine.of(
              m,
              line++,
              item("SKU-MVL-" + n + "-" + line, catA, BigDecimal.ZERO, true),
              null,
              locX,
              null,
              null,
  ...
  void byWarehouse_sumsQtyAndValue_preservesEmptyActiveWarehouse_excludesInactive() {
    ...
    assertThat(rows)
        .extracting(WarehouseStockResponse::warehouseId)
  ...
  void movementsByType_countsConfirmedOnly_allEnumValuesPresent() {
    ...
    movement(
        MovementType.RECEIPT, LocalDate.of(2026, 2, 1), false, BigDecimal.valueOf(99)); // DRAFT 제외
    authenticated();

    List<MovementTypeCountResponse> rows = service.getMovementsByType();
    assertThat(rows)
        .extracting(MovementTypeCountResponse::movementType)
  ...
  void monthlyMovements_fills12Months_perType_confirmedTargetYearOnly() {
    ...
    confirmedMovement(
        ...
        BigDecimal.valueOf(4)); // 2라인 → qty 7, 건수 1
    confirmedMovement(MovementType.ISSUE, LocalDate.of(2026, 1, 20), BigDecimal.valueOf(2));
    confirmedMovement(
        MovementType.ADJUSTMENT, LocalDate.of(2025, 12, 1), BigDecimal.valueOf(8)); // 다른 연도
    movement(
        MovementType.RECEIPT, LocalDate.of(2026, 2, 1), false, BigDecimal.valueOf(99)); // DRAFT
    authenticated();

    List<MonthlyMovementByTypeResponse> rows = service.getMonthlyMovements(2026);
    assertThat(rows)
        .extracting(MonthlyMovementByTypeResponse::movementType)
  ...
  void lowStock_includesBoundaryAndZeroStock_excludesAboveAndInactive() {
    ...
    Location locLow =
        locationRepository.save(Location.of(whLow, "LOC-LOW", "위치", null, LocationType.BIN));
    ...
    assertThat(rows)
        .extracting(LowStockItemResponse::sku)
  ...
  private List<MonthlyMovementResponse> series(
      List<MonthlyMovementByTypeResponse> rows, MovementType type) {
    return rows.stream().filter(r -> r.movementType() == type).findFirst().orElseThrow().months();
  }

  private void assertMonth(
      List<MonthlyMovementResponse> months, int month, long count, BigDecimal qty) {
    MonthlyMovementResponse m =
        months.stream().filter(r -> r.month() == month).findFirst().orElseThrow();
