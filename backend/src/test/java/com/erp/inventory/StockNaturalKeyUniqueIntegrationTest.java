...
/**
 * stock 자연키 UNIQUE가 lot_no/serial_no NULL을 동일하게 취급(NULLS NOT DISTINCT)하는지 검증. 비-로트/시리얼 품목은
 * lot/serial이 NULL인데, 과거 UNIQUE는 NULL을 distinct로 봐 같은 (item, location)에 중복 stock 행을 허용했다 — 동시 입고 시
 * 중복·이후 단건조회 깨짐의 원인.
 */
@Transactional
class StockNaturalKeyUniqueIntegrationTest extends AbstractIntegrationTest {
  ...
  void duplicateNaturalKey_withNullLotAndSerial_isRejected() {
    ...
    Location loc =
        locationRepository.save(Location.of(wh, "LOC-DUP", "위치", null, LocationType.BIN));
