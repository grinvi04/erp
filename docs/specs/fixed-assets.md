...
- **AC-7 (처분, 정상):** WHEN 자산을 매각/폐기, the system SHALL 처분일까지 상각을 catch-up 반영(처분월 미상각: 취득월 포함·처분월 제외, 월할)한 뒤 장부가액 제거·처분손익 균형 분개를 만들고 상태=처분으로 전이하며 이후 상각에서 제외. (catch-up은 처분월 시작일 이전에 끝난 OPEN 회계기간 중 미처리분만 순차 전기; 마감 기간은 건너뜀.)
