...
## 6. Open Questions
- (없음 — 온더플라이 한도·신규 환입계정·수동·환입후 재배분으로 확정.)

### 알려진 한계 (회계기간 마감규율 전제 — 손상 인식·감가상각과 동일)
- 재개방된 **이전 기간**(손상 인식 기간보다 앞선 OPEN 기간)에 환입하면 시간 역전 분개가 가능하다 — 기간을 순서대로 마감하는 규율을 전제로 한다(손상 인식·월 상각도 동일 가정).
- `catchUpThroughPeriod`는 마감(CLOSED)된 미상각 기간을 WARN 후 건너뛰므로, 그런 기간이 있으면 환입 측정 `bookValueBefore`가 실제보다 높아 환입액이 과소될 수 있다(처분 catch-up과 동일 성질).
- 같은 기간에 인식한 손상은 환입하지 않는다(F053으로 차단) — IAS 36상 환입은 이전 기간 손상 대상.
